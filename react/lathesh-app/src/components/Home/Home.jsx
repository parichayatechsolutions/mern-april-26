        ...
        <>
            <div className="flex flex-row pt-25 px-60">
                <div className="w-1/2 relative right-7">
                    <div className="relative font-bold font-[cursive] border-3 border-solid border-black rounded-2xl shadow-[8px_8px_5px_black] py-8 px-2.5 -rotate-1 hover:transition-all delay-300 ease-out hover:shadow-[15px_16px_black] z-10 bg-amber-50 opacity-90 hover:bg-blue-200 hover:rotate-0 ">
                    ...
                    {
                        stickerCards.map((cd,index) => {
                            return (
                                    <div key={index} className={`{ absolute font-bold font-[cursive] border-2 border-solid border-black rounded-2xl shadow-[5px_5px_black] py-0.5 px-1.75 -rotate-5 transition-all delay-500 ease-in text-white ${cd.position} bg-amber-800 }`}><strong>{cd.name}</strong></div>
